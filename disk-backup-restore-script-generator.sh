...
echo "sfdisk --dump "$DISK" > \"$TARGET_DIR/$PARTITION_TABLE_FILE\"" >> $BACKUP_SCRIPT
...
for PART in $PARTITIONS; do
    ...
    case $FS_TYPE in
		    ...
		    echo "partclone.${FS_TYPE} -c -s '$PART_PATH' | $COMPRESSOR > '$TARGET_DIR/$BACKUP_PART_NAME'" >> "$BACKUP_SCRIPT"
		    echo "cat \"\$SOURCE_DIR/$BACKUP_PART_NAME\" | $DECOMPRESS | partclone.$FS_TYPE --restore -c -o \"\$TARGET_DEV\$TARGET_DEV_SEP\$PART_NR\"" >> "$RESTORE_SCRIPT"
