...
VERIFY_SCRIPT=$TARGET_DIR/verify-backup.sh
...
for PART in $PARTITIONS; do
    ...
    case $FS_TYPE in
		    ...
		    ok_guard "$TARGET_DIR/$BACKUP_PART_NAME.ok" "partclone.${FS_TYPE} \$PARTCLONE_CLONE_ARGS -c -s '$PART_PATH' -x $COMPRESSOR -O '$TARGET_DIR/$BACKUP_PART_NAME'" >> "$BACKUP_SCRIPT"
		    ...
		    echo "partclone.chkimg "$BACKUP_DIR"'/$BACKUP_PART_NAME'" >> "$VERIFY_SCRIPT"

		    ;;
            *) 
		    FS_TYPE="dd" 
		    BACKUP_PART_NAME="$PART_NR.$FS_TYPE.$COMPRESSOR"
		    echo "partclone.${FS_TYPE} -s '$PART_PATH' -x $COMPRESSOR -O '$TARGET_DIR/$BACKUP_PART_NAME'" >> "$BACKUP_SCRIPT"
                    echo "cat \"\$SOURCE_DIR/$BACKUP_PART_NAME\" | $DECOMPRESS | partclone.$FS_TYPE ${RECOVER_FLAGS} \"\${TARGET_DEV}\${TARGET_DEV_SEP}$PART_NR\"" >> "$RESTORE_SCRIPT"
		    echo "partclone.chkimg "$BACKUP_DIR"'/$BACKUP_PART_NAME'" >> "$VERIFY_SCRIPT"
		    ;;
    esac
